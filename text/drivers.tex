...
Querying every such device in a loop would be inefficient both in terms of power and performance and so devices can raise \keyword{interrupts}.
...
\fixme{Reference networking chapter once it exists}
...
The RISC-V \keyword[abbreviation=PLIC]{platform-local interrupt controller} (PLIC) handles all interrupts coming from devices and forwards them to the core.
...
The switcher will then invoke the \keyword{scheduler}, which will query the PLIC to see which interrupts have fired and wake more any threads that were waiting for them.
