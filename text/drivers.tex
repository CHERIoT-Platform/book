...
\amanda{Locations or location?  Is it possible for a physical device to have more than one location?}
...
\begin{cxxsnippet}
[,cpp]
\amanda{Is this line ("[,cpp]" supposed to be here?}
do
{
...
\end{cxxsnippet}
\amanda{For reasons I do not understand, the both the PDF and HTML versions have a blank line between "//Handle interrupt here" and "interrupt_complete(...) which looks very weird.}
...
For example, the scheduler is the only component that has direct access to the interrupt controller, while the memory allocator is the only component that has full access to the revoker.
...
These are added \textem{in this order}, which makes it possible for code in the more specialised directories to \c{#include_next} more generic versions of the files or to add files that are found in preference to the generic versions.
\amanda{It is possible that that was supposed to be three different options instead of two.  If so, we need to find a clearer way to express what the first one is supposed to be.}
