...
The \lua{"library"} target, for example, sets itself as a non-default target.
...
\macrodoc{DEFINE_STATIC_SEALED_VALUE}
...
\amanda{In this functiondoc, in the 1st paragraph, "Space" should not be capitalized.}
\amanda{In this functiondoc, in the 4th paragraph, "this will also set the permissions of passed capability reference to Permissions" should either be "a passed capability reference" or "passed capability references"}
\amanda{In this functiondoc, "less" should be "fewer."  Permissions are discrete and countable.}
...
\amanda{In this functiondoc, "check_pointer" should be formatted as a command.}
...
This can be adopted by adding \lua{add_deps("unwind_error_handler")} to your compartment's target in \file{xmake.lua}.
