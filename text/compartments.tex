...
\amanda{In the first paragraph of this box, the code uses "space" but the text uses "Space" and they should match.  IMO it's better to uncapitalize the version in the text since a later paragraph uses "space."}
\amanda{In the first sentence of the fourth paragraph of this box, the text reads "set the permissions of passed capability reference" and should read "...of the passed..."}
\amanda{In the second sentence of the fourth paragraph of this box, the text reads "ask for less permissions..." Permissions are discrete and countable things, and therefore it should read "ask for fewer permissions..."}
...
\amanda{Within this box, "check_pointer" should be formatted as code.}

Alternatively, you can use the ephemeral claim mechanism described in \ref{heap_claim} to ensure that a pointer is either a pointer that cannot be freed, or to ensure that it remains live until the next cross-compartment call.
...
First, it uses \c{heap_claim_ephemeral} to prevent concurrent deallocation.
...
The malicious caller from the previous example would see a return value of \c{-ENOTENOUGHSTACK} and your code would not be invoked.
