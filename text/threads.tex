...
There is an example thread pool in \file{lib/thread_pool} in the RTOS SDK, with its interface defined in the \file{thread_pool.h} header, that you can either use directly or use as inspiration for your own design, if you want to create new thread-like contexts.
...
\amanda{Within this functiondoc, there is the text "Those contexts are very restricted in what it can do," which should be "... very restricted in what they can do,"}
...
\amanda{Within this functiondoc, in the third paragraph, there is the text "A call of thread_sleep is with a timeout of zero..." The word "is" should be removed, leaving "A call of thread_sleep with a timeout of zero..."}
\amanda{Within this functiondoc, in the third paragraph, the term "cross-domain" is used.  Did you mean "cross-compartment"?  "Domains" have not previously been mentioned in the book.}
...
\amanda{Within this functiondoc, there is the text "futex[_timed]_wait" but there has been no mention of just plain "futex_wait" and will not be for many paragraphs.  futex_wait should be mentioned at least briefly somewhere around here.}
...
\amanda{The dashes I added are not ideal, since "wait to work" is a valid phrase, but not setting it apart makes the sentence harder to read.  An alternate solution is to put quotes around "wait to work" and I'm open to either method.}
...
\amanda{Should event-groups be plural for FreeRTOS's APIs?}
...
\amanda{In this functiondoc, the third paragraph contains text that is emphasized via asterisks instead of textem}
\amanda{In this functiondoc, the third paragraph ends with the text "if different threads clear bits different bits in the waited set."  This should read "...clear different bits in the waited set."}
...
\amanda{Within this functiondoc, the last paragraph contains the text "and so will avoid upgrading the locks if freeing the queue would fail."  The word "so" is unnecessary; the text should read "and will avoid upgrading the locks if freeing the queue would fail."}
...
\amanda{Within this functiondoc there is text that is empathisized with asterisks rather than textem.}
...
\amanda{The quotes used are an unusual choice and look odd in both the HTML and PDF versions.  Is this for epub formatting, or just an unconventional stylistic choice?  If it's not strictly necessary, I would recommend either using '' or formatting it as code.}
This is typically an on-stack array.

\functiondoc{multiwaiter_wait}
\amanda{Within this functiondoc, error codes are not formatted the same way that error codes in previous functiondocs have been formatted.}
...
\amanda{Er, non-full, or non-empty?}
...
\amanda{Receiving or sending?  And is "full" the correct word, or "non-empty"?}
