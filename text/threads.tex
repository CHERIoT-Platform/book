...
\phil{ Maybe picky, but as a compartment the Scheduler doesn't run at all, but different threads may enter and run in the Scheduler compartment.  Keeping the distinction between compartments and threads seems important.}
...
\phil{ So thread_get_id is not a cross-compartment call (because it just reads from the register area of the current thread), but thread-count() is, and so has bigger impact (like it could release an ephemeral heap claim ?)}
...
\phil{ Something that wasn't obvious to me, and doesn't really come across here is that when you create timeout of n ticks it means "n ticks from when it was created" - I can't for example create a timeout once of 10 ticks and pass it to an API every time I want to wait for another 10 ticks. }
...
This allows a high-priority threads to allow other threads to run, but continue using the CPU if no other thread is runnable.
\hugo{ typo }
...
In both cases, even if no other threads are runnable, you have no useful work to do for a bit.
\hugo{ nitpick }
