...
\amanda{Was the word "mode" intentional or was it supposed to be "more"?}
...
The \file{FreeRTOS-Compat/semphr.h} header provides wrappers for the various types.
\amanda{Various types of what?}
...
\amanda{Are both of these words supposed to be "callee" or should one be "caller"?  If it's correct, consider "on behalf of itself" or "on its own behalf" instead.}
...
A futex-based lock is very cheap to acquire in the uncontended case; it requires a single atomic compare-and-swap instruction, which may be a function call to a library routine that runs with interrupts disabled if the hardware does not support atomics.
