...
Phil> showing how on only the Firewall has access to the Ethernet hardware would be nice.
...
The SNTP compartment and the time helpers library share a pre-shared object (see \ref{_sharing_globals_between_compartments}), contains the UNIX timestamp at the time of the last NTP update, the cycle time of the last update, and the current epoch.
...
Phil>Consider removing the commented out stack_size line
...
Phil> "which will retry if it fails." ?
...
The CHERIoT MQTT interface doesn't support unencrypted connections and so connecting to a server requires everything that you needed for a TLS connection.
...
This API and takes quite a lot of arguments.
...
The \flag{-j} flag should be coped as-is, this finds the JSON file that the linker created with the audit report for the firmware image.
...
These are all in the \file{mqtt.rego} file in the example and so you can add \flag{-m mqtt.rego} to your \command{cheriot-audit} command line to use them.
...
Change the timeout for one of the \c{mqtt_run} calls so and read a switch or UART to determine when to call \c{network_inject_fault}.
