...
Each of these stats with a JSON object literal for the capability that we expect to find (the one that we saw earlier using \command{cheriot-audit} for introspection) and then uses the \rego{in} operator to check that this object is part of the array.
...
The LCD shows a CPU usage graph at the top and a heap-memory usage graph at the bottom.
