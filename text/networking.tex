...
\amanda{Okay, I am not 100% confident of this change, because my code-parsing skills are subpar.  But it does look like there's only one number of seconds each time.  If you actually meant "second number of seconds" can we change it to "new number of seconds" or something so that it's easier to parse?}
...
\end{console}
\amanda{Out of curiousity, where does the text "Network test: " come from?  I can see all the rest of it (except "Starting network stack" but I assume that's baked into the network_start}
...
\end{console}
\amanda{The PDF version that I downloaded (which is several commits out of date, so this may change) has the ASCII art split across two pages.  I recommend tagging this like a table so that it does not split.}

\functiondoc{network_socket_receive_preallocated}
\amanda{The first paragraph of this functiondoc uses "data are" (twice!)  While "data" is technically plural, in usual use it can be treated as either singular or plural, and this paragraph reads very weirdly because most people would read "data" as singular in this case. I recommend either changing it to "data is" or rewriting it to avoid using a to-be verb, for example "This will block until it receives data or the timeout expires. If it receives data, the data will be stored in the provided buffer."}
...
\amanda{Again, in this functiondoc, data is treated as a plural noun when this use case should treat it as singular. I recommend changing "have" to "has" but you could also use something like "until the data finishes sending or the timeout expires."}
...
\amanda{I do not like the colon, because the second sentence ("The firewall...") is part of the threat model but not part of the phrase after the colon-- it's its own phrase by virtue of being its own sentence.  However, trying to shove the second sentence into the second half of the first sentence would be clunky and awkward.  I recommend 1) re-writing the beginning of the first sentence to remove the colon, or 2) turning this into an itemized list, which may require re-writing the third sentence to make it fit the list better.}
...
\amanda{There is a word missing after "different."  I suspect it is "connections" but it might be "compartments" or "packets" or something else entirely.}
...
\amanda{Technically you generate the key pair, which includes the private key.}
...
\amanda{Six years as of the time of writing.  You can't assume your reader will be reading this is 2025.}
...
The MQTT, TLS, and TCP states are visible only to the compartment that owns them.
\amanda{This should possibly be "...owns each of them." if they can be owned by different compartments.}
...
\amanda{Er, maybe I'm missing something but the return code is greater than 0 (i.e. non-negative, therefore a packet ID, therefore not an error) but we print that we had an error.  Shouldn't it be "ret < 0"?}

\functiondoc{mqtt_run}
\amanda{Again, the code appears to print an error message if the return code is greater than zero but errors are negative returns.  If you say this is correct I will believe you but I would like confirmation of it.}
...
\amanda{Again, the code appears to print an error message if the return code is greater than zero but errors are negative returns.  If you say this is correct I will believe you but I would like confirmation of it. (It is happening often enough that I have to assume I'm missing something, but I would be remiss if I didn't point it out and it turned out to be wrong.)}
...
\amanda{Wouldn't this leave a network socket open forever?  Presumably they *sometimes* disconnect on purpose.}
...
You pass this to the \flag{--module} (or \flag{-m}) argument to \command{cheriot-audit}.
\amanda{"to" the --module argument or "with" the --module argument?}
For the rest of this section, we'll use \command{cheriot-audit} the \ref{mqtt} example.
From the \file{examples/mqtt} directory, you will need to run a command like this:
\amanda{There are words missing from this sentence but I don't know what they are.  "We'll use cheriot-audit to audit the mqtt example" maybe?  "We'll use the mqtt example with cheriot-audit"?  "We'll run cheriot-audit on the mqtt example"?}
...
\amanda{Presumably the MQTT example also opens a firewall rule that allows it to communicate with the Mosquitto server.  The phrase "opens a firewall rule" isn't really necessary as it was covered in detail in an earlier section, but if you're going to use it, use it for both compartments.}
...
\amanda{Is "reset state machine" what you meant, or should there be "reset state of the machine"?  This is the first time you've used "machine" anywhere and I'm uncertain if it's truly intended.}
