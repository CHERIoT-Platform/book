...
== Replacing a real-time control loop
...
A function that has the `[[cheri::interrupt_state(disabled)]]` attribute will run with interrupts disabled and so has exclusive use of the core until it yields.
...
<<threads>> discusses the various ways for a thread to block.
