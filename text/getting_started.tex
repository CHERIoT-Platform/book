...
This creates an ephemeral instance. You can create persistent instances with \command{docker create} instead of docker run.
...
\amanda{I think.  If the toolchain doesn't contain the compiler, linker, etc then feel free to ignore this change.}
...
\amanda{This sentence made me laugh very hard.  I love it!}
...
It includes a mature C/C++ front end and a component that transforms C and C++ (and Objective-C) into LLVM IR.
...
\amanda{This sentence no verb.  Possibly there was supposed to be a list here?  If the list is just llvm-objdump and llvm-objcopy, it should probably read something like "LLVM also provides some other components that are useful for development."}
...
\amanda{I was unable to find a definitive answer to whether vim should be capitalized or not, but you should be consistent and either capitalize the first instance of it ("...if you install vim") or uncapitalize the second instance (..."in Vim to install it.")}
...
the first of which should be available in 2025.
...
\amanda{I found "the first two" and "the next two" to be confusing, because I was expecting two lines for each.  May I suggest phrasing this as "The first add_deps adds two libraries provided by the RTOS....The next add_deps adds the two compartments that we definied earlier."}

Not all of the metadata that we set can be defined in the declarative syntax of \command{xmake}, so we have to implement a function using the \lua{on_load} hook to set the remaining properties.
...
\end{description}
\amanda{Unless the order the properties are specified is critical (in which case, state that for the reader), please make the properties in the code be in the same order as the description of them.  I would re-arrange this list, but it's in alphabetical order, so I suspect it may be better to re-order them in the code.}
...
Extra mount locations can be added to the \file{.devcontainer/devcontainer.json} file.
