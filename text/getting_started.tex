...
Phil> Just to note this doesn't seem to render well in the HTML version

The \lua{set_project} call gives a name to the project.
Phil> Renders as "The call..." in the HTML version
...
You can set a default and we use \lua{"sail"} here the simulator build from our Sail formal model of the ISA.
...
Phil> Renders in html as "The property" - \lua seems to become invisible
...
Phil> Maybe with adding that this has to be at the top level of the JSON (I know that's implied by the indentation, but its silently ignored if you get it inside says customizations)
\begin{jsonsnippet}
  "mounts": [
