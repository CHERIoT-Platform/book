...
\begin{table}
	...
	\phil{ the term "untagged capability" doesn't immediately convey that what you get back is a capability that will fault if you try to use it - i.e. you have to use an is_valid() check on the result to see if the call worked or not. }}}
...
This is done with \c{__builtin_cheri_seal}, which takes an authorising capability (something with the permit-seal permission) as the second argument and sets the object type of the result to the value of the sealing capability.
\hugo{ Using address here is confusing. Could alternatively use 'the value of the address field', but this is a mouthful. }
...
\hugo{ Using permission over capablity here, makes it slightly easier to follow. }
