...
If a function is defined while compiling a compilation unit belonging to a different compartment, the compiler will raise an error.
...
For example, if you change the compartment name in \ref{lst:simpleexport} to \c{"wrong_compartment"}, you will see the following error when compiling \file{compartment.cc}, which contains the definition of this function:
...
This attribute can also be used via the \c{__cheriot_callback} macro, which allows it to be defined away when targeting other platforms.
...
The library is called from the compartment so you should see the stack pointer move, but the bounds will remain the same.
...
\begin{description}
	...
	\item[tag=inherit]{ The interrupt state is unchanged (inherited from the caller) when invoking this function.}
\end{description}
...
\codelisting[filename=examples/interrupts_disabled/interrupts.cc,marker=high,label=lst:highpriorityinterruptsdisabled,caption="A high-priority thread that is starved by an interrupts-disabled function called from a low-priority thread."]{}
...
\begin{description}
	\item[tag=address]{The current address that's used when the capability is used as a pointer.}
...
Setting the object type for sealed capabilities is more complex and requires a second capability that authorises sealing.
The address field for capabilities with permit-seal or permit-unseal permissions refers to the object-type space, rather than the memory address space.
The \c{__builtin_cheri_seal} function takes an authorising capability (something with the permit-seal permission) as the second argument and sets the object type of the result to the address of the sealing capability.
Conversely, \c{__builtin_cheri_unseal} uses a capability with the permit-unseal permission and an address matching the object type to restore the original unsealed value.
...
\begin{note}
	Although most of the macros in \file{cheri-builtins.h} match the names of the underlying builtins, the permissions macros follow the CHERIoT RTOS coding convention of avoiding abbreviations and so use \c{permissions} instead of \c{perms}.
...
\amanda{In the PDF, Listing 22 (this one) comes before Listing 21 ("Pretty-printing a capability using the C builtin wrappers".  I assume this is because Listing 22 is shorter and will fit on the page, but Listing 21 is longer so it prints at the top of the next page.  I do not have any idea how one would go about fixing this, but it looks very odd.  It does not help that the next line "When you run this example, you should see something like this" is the last line on the page, so the "something like this" that you should see comes AFTER the code listing.}
...
\amanda{In the PDF, this listing is pushed to the next page, but the example output is not finished, so it comes right in the middle of it.}
\amanda{I almost want to suggest a change in formatting so that code listings have boxes around them, not unlike functiondocs.  That would make it much clearer what part is code and what part is output/text, and less confusing when they interrupt text that ought to be kept together. UPDATE: I will later (in threads.tex) suggest changing \console{} code to keep it together the same way that functiondocs and code listings are, and I think this is a better solution overall.}
