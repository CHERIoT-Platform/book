...
1 error generated.
\end{console}
\amanda{It is somewhat confusing that the error occurs in entry.cc when the most recent example used compartment.cc, particularly because the paragraph before this used the phrase "in this example."  I would recommend making them match.}
...
\amanda{Is there a reason for the newline in the last line of this code?  It's inconsistent with all the other listings you've had so far.}
...
For example, the RTOS used to provide a library that implemented all of the helpers for atomic operations was later split into separate libraries for different sized objects, allowing code to link only the atomic operations for types that it uses.
...
\amanda{The text above the code explaining interrupt_state(disabled) is "cheri::" but the text in the code is "cheriot::"  Should one of these be changed?}
