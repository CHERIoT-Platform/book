...
\amanda{There is technically nothing wrong with "turned on on" but I hate it anyway.  Can we change "turned on" to "activated" and make it read "...to be activated on a per-compartment basis..."?  I freely admit that this is my own personal bias and it's not my book, so "No, leave it alone" is a perfectly valid answer.}
...
\amanda{This is clunky and awkward.  Would "You must also enable debugging support in your compartment or library by adding..." still be accurate?}
...
This class is designed to avoid needing heap allocation or large amounts of stack space and so is intentionally less flexible than a general-purpose formatting library.
