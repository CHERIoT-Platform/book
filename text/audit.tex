...
\amanda{In the example below, you used a -q flag.  If the -q is necessary, it should be noted here, just like the -j and -m flags are.}
\end{description}
...
\amanda{The PDF version displays rego{} stuff with a different font, but while the html file includes a <span class='rego'> markup, there is no "rego" class defined in book.css.  If you have no specific plan for how that class looks, I would recommend making it look the same as \c{} or \command{} does.}
...
\amanda{I'm pretty sure that in the above paragraph, "contents", "kind", and "sealing type" should all be formatted differently, but I am unsure if they should be \c{} or \rego{}}
...
\amanda{Should "core-local" and "platform-level" have formatting?}
...
\amanda{Did you mean "only the allocator should have write access to it"?  As written, it sounds like nothing else should read it, safe or not.}
