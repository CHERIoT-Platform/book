...
\hugo{The CHERIoT software model provides \keyword{shared libraries} for cases where a change of context isn't required.}
...
\hugo{Or you can say that users can provide their own logic for controlling their IoT device, but only your code may connect to the network stack if users want to sign the image with a key that authorises the release of a private key.}
