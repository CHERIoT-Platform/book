...
This is often solved in capabilities by adding a layer of indirection; you hold a capability that authorises you to make use of a different capability in some table and your rights can be removed by deleting the capability from the table.
\hugo{Took me a second pass not to muddle the capabilities. This suggestion may differentiate them a little}
...
\hugo{nit: It may make sense use 4 bytes instead of a word. Some may be coming from other ISAs where a word is a different number of bytes, and so will have to engage brain a little more than necessary when bytes are discussed below.}
...
\hugo{nit: The registers are rendered uppercase (i.e. CA1) inline but lowercase in the code blocks.}
...
\hugo{
  Currently, the first time the bounds encoding and it's implications are discussed is section 5.9,
  with it hinted at in section 5.7 ('bounds set exact' built in).
  Similarly to how the permissions being compressed is briefly mentioned below,
  I think it would be nice to mention the bounds being compressed and with a link to find more information in section 5.9.

  In the past, bugs due to in-exact bounds were the ones that took collogues new to the project the longest to debug.
}
...
\hugo{nit: Labelling the start and end bits of the fields in the diagram would make the engineer in me happy. But not needed for explanation.}
...
This means that while removing the primary global permission gives a shallow no-capture pointer.
Code that it is passed to can store it in registers and on the stack but nowhere else), however capabilities loaded through it may still allow loading other capabilities with global set unless load-global is removed.
...
\hugo{I think a few more words here goes along way in avoiding confusion around global vs load-global.}
...
\hugo{
The concept of sealing is really well explained, but the implementation is a little hard to follow.
I would dedicate a sentence to where the sealing and unsealing capabilities hold their value.
Changing out the last two sentences to something like:

Both sealing and unsealing capabilities have values where an ordinary memory capability would have an address.
When a capability is sealed, it's object type is set to the value of the sealing capability.
It can then can only be unsealed by an unseal capability with a matching value.
}
...
The CHERIoT software model provides \keyword{shared libraries} for cases where a change of context isn't required.
...
Or you can say that users can provide their own logic for controlling their IoT device, but only your code may connect to the network stack if users want to sign the image with a key that authorises the release of a private key.
