...
Phil> Might be a good place to introduce the concept of creating derived capabilities - You can make copies of a key that ether unlocks the same or a subset of the locks, but you can't create a key from nothing. 
...
Phil> Being adjacent hints pointers are unique, when they could of course be overlapping .
...
Phil> a bit of a nit, but "Different fields" kind of implies that a capability could refer to several disjoint areas, like the first and third field.
...
Phil> Feels odd that Global is the only one not described in this para

The dependent permissions (listed in \ref{tbl:dependent_permissions}) provide more fine-grained control.

Phil> I've always struggled with this section a bit - what is it that they depend on ?
Phil> Is it that their meaning changes and/or they can only be interpreted in conjunction with a primary permission ?  
Phil> Maybe say that they defining what you can do with other capabilities stored in the memory referred to by the capability. 
...
Phil> It might be, but that's not what is then described in the table
Phil> It doesn't help that there is a "Load" capability, a "Store" capability and a "Load / Store capability" capability that reads a bit as if it's referring to either of the first two
Phil> Also why are "Load Global" and "Load Mutable" not called "Load Global Capability" capability and "Load Mutable Capability" capability to match ?  
...
Phil> There's a fair bit of inconsistent in this section of using hyphenated names in the text but not the table. 
...
Phil> This is the first mention of auditing as a concept I think - might need a forward reference ? 
