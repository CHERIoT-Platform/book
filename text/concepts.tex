...
\phil{ Might be a good place to introduce the concept of creating derived capabilities - You can make copies of a key that ether unlocks the same or a subset of the locks, but you can't create a key from something that just happens to look like a key. }
...
\phil{ I was expecting this to go on and mention how some capabilities can be tagged as being valid or not ?}
...
\phil{ I've always struggled with this section a bit and the concept of "dependent permissions" - what is it that they depend on ?}
\phil{ Is it that their meaning changes and/or they can only be interpreted in conjunction with a primary permission ?  }
\phil{ It feels like, with the exception of Access System Registers, that they defining what you can do with other capabilities stored in the memory referred to by the capability that holds them - so maybe "capability permissions" would be a better description.}
...
\phil{ Why are "Load Global" and "Load Mutable" not called "Load Global Capability" and "Load Mutable Capability" to match "Load / Store Capability"?}
\phil{ "Store Local" would be better as "Store Local Only"  }
...
\phil{ Are the *'s meant to be a Markdown command ?}
