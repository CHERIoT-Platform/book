...
\amanda{The word "some" here is confusing.  The next sentence makes it sound like it's a specific privileged mode, so I'm making the change to reflect that, but I am not 100% confident that it's correct.  I'd actually recommend re-writing both sentences to be something like "This register can be modified only by code running with the Access System Registers permission..." but, you know, accurate.}
...
Or you can say that users can provide their own logic for controlling their IoT device, but require that only compartments that you trust can have permission to connect to your cloud servers.
