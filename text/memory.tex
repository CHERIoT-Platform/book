...
If you need to ensure that an allocation remains valid for a brief, scoped, period then \c{heap_claim_fast} may be more useful.
This function places an ephemeral claim on one or two objects.

\begin{warning}
	The \c{heap_claim_fast} function is poorly named (and will probably be renamed something like \c{heap_claim_ephemeral} in a future version).
	It is not just a fast version of \c{heap_claim}, it is a different function with different guarantees.
\end{warning}

\functiondoc{heap_claim_fast}

Every thread has two \keyword{hazard slots} that can hold pointers.
The \c{heap_claim_fast} function manages these two slots.
These are cleared on every cross-compartment call and can be cleared explicitly by passing \c{NULL} to \c{heap_claim_fast}.
...
\begin{caution}
	Any claim applied with \c{heap_claim_fast} is lost on \textem{any} cross-compartment call.
	This includes any blocking operation, which will invoke the scheduler.
	In general, do not use \c{heap_claim_fast} for anything other than a local read or write of a single object.
\end{caution}

The \c{heap_claim_fast} API is intended for very brief accesses to objects.
