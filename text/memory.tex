...
Phil> Might be just me, but took me a while to grok what the load-filter is and "when it is loaded" means in this context (was thinking about the Load permission).  So any attempt to put the capability into a register so it can be used will clear the tag if (some ?) of its range is set in the corresponding area of the revocation bit map ?  
...
Phil> ct tags in the section title look messy and seem unnecessary 
...
Phil> Should the first line of text in the function doc be "interface to the ephemeral claims mechanism" rather that "fast claims mechanism" ?
